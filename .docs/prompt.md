...
关闭模式不注入任何本插件提示词。Host 从请求的有效系统提示快照读取模式：显式 `system` 字段优先；否则读取 `messages` 中由 `@deepseek-ai/dsh-system-prompt` 提供的最新非空 system 消息。没有来源元数据的调用使用 system 角色消息。空尾节点不参与选择，最新快照没有标记即关闭转写，不能回溯启用历史模式。用户和 Assistant 正文中的标记不参与判断。辅助 `purpose` 调用保持原流。
