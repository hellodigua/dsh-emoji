...
2. Agent 在面向用户的自然语言正文中，根据用户提示选择表情；智能模式只在表情能改善表达时使用，允许零张；高频模式要求所有对话回复都加入一个匹配情绪的 marker，并把它放在最相关的句子或短段落后。智能模式最多保留 3 个 marker，高频模式最多保留 4 个，相同 key 可以在正文不同位置重复；作为字面正文内容的 Unicode emoji 仍可保留。
