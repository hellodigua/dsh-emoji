...
5. `src/assets.ts` 用 catalog 做白名单查询并从包内 `assets/emoji/deepseek/` 返回 PNG。
6. `src/client/index.ts` 只对 dsh-emoji 路由图片应用 `2em` 行内样式。
...
- `src/settings.ts` 注册 Settings namespace，并提供 loopback-only 的 get/save/reset RPC。
