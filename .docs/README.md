...
`dsh-emoji` 是一个独立的 DeepSeek Harness Profile Bundle，为 Web Assistant 回复提供受控 Unicode 表情选择、LLM 流确定性转写、PNG 路由、行内展示和实时频率配置。当前运行时内置 40 张透明蓝色正面鲸鱼表情，不修改 DSH core，也不实现用户输入选择器或 TUI 渲染。
...
- system prompt 使用英文技术默认值，以紧凑的 `Unicode=English/中文` 目录提供 42 个合法输入字符，对应 40 个核心语义 key；模型不接触内部 key，也不注册表情 Function Call。
- Host 用 global `llm/stream` 监听跨过运行时 scope，根据请求内模式标记在安全的 `text-delta` 边界增量转写受控 Unicode 表情，并让最终 text block 与累计增量保持一致，不触发第二次模型调用。
...
- `src/reaction-emoji.ts`：40 个规范 Unicode 字符、两个常见输入别名与内部 key 的明确映射。
- `src/reactions.ts`：受控 Unicode 表情与插件图片的 Markdown 安全转写。
...
- `tests/`：catalog、检索、Unicode 表情流、路由、Client 生命周期、设置和包结构验证。
...
- `auto` 的触发时机以及 `frequent` 的强制频率仍依赖模型遵循提示词，插件不会在模型漏掉表情时自动补图。
- 用户定义的表情偏好、插入位置和跳过场景依赖模型遵循提示词；程序只负责 Unicode 白名单、Markdown 边界和分档数量上限，不判断回答情绪。
...
- [System Prompt 中文对照](prompt.md)：运行时英文提示词的中文核对稿、动态拼装顺序、模式差异和 Unicode 白名单。
...
- 在真实 rc.7 npm 类型与运行时包图上，typecheck、测试、build 和 pack dry-run 均通过；设置卡片覆盖 keyed slot namespace、官方折叠箭头、展开态、悬停态、键盘焦点样式和内置包技术标识隐藏规则，转写另覆盖 40 项规范映射、两个输入别名、多码点 grapheme、未知变体、双冒号普通文本、CommonMark 代码／链接边界与相邻表情。
