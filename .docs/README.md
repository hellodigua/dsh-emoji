...
- `src/request-mode.ts`：从有效系统提示快照读取本次请求的模式。
- `src/settings-routes.ts`：共享 Connection API 上的精确设置路由和线协议校验。
...
- 设置路由使用 DSH Connection 的 Host/Origin 与 Cookie 认证，Client 依照 `connection.isLoopback` 开放设置入口；只读写本插件命名空间。
