...
*/
const CANONICAL_REACTION_EMOJI_BY_KEY = Object.freeze({
	happy: "😊",
	sad: "😢",
	confused: "😕",
	watching: "👀",
...
const CANONICAL_REACTION_EMOJIS = Object.freeze(EMOJIS.map((emoji) => ({
	emoji: CANONICAL_REACTION_EMOJI_BY_KEY[emoji.key],
	catalog: emoji
})));
/** 常见模型输出的输入别名；不新增内部 key，也不改变持久化图片 alt。 */
const REACTION_EMOJI_ALIASES = Object.freeze([{
	emoji: "😄",
	key: "laughing"
}, {
	emoji: "🙂",
...
const catalogByKey = new Map(EMOJIS.map((emoji) => [emoji.key, emoji]));
/** Host 与 system prompt 接受的全部字面 Unicode 表情。 */
const ACCEPTED_REACTION_EMOJIS = Object.freeze([...CANONICAL_REACTION_EMOJIS, ...REACTION_EMOJI_ALIASES.map(({ emoji, key }) => {
	const catalog = catalogByKey.get(key);
	if (catalog === void 0) throw new Error(`dsh-emoji: alias points to missing key ${key}`);
	return {
		emoji,
		catalog
	};
})]);
const catalogByUnicode = new Map(ACCEPTED_REACTION_EMOJIS.map(({ emoji, catalog }) => [emoji, catalog]));
/** 按规范 Unicode 或明确输入别名查找内部 catalog；未知字符不做近似匹配。 */
function catalogEmojiByUnicode(emoji) {
	return catalogByUnicode.get(emoji);
}
/** 按内部 catalog 条目取得持久化到消息 alt 文本的规范 Unicode 表情。 */
function canonicalReactionEmoji(emoji) {
	const unicode = CANONICAL_REACTION_EMOJI_BY_KEY[emoji.key];
	if (unicode === void 0) throw new Error(`dsh-emoji: missing canonical Unicode mapping for ${emoji.key}`);
	return unicode;
}
//#endregion
//#region src/reactions.ts
/** System prompt 内用于把一次请求绑定到确定频率策略的稳定前缀。 */
const REACTION_PROMPT_PREFIX = "[dsh-inline-reaction:mode=";
...
function graphemesByStart(text) {
...
function rewritePlainText(text, state, imageUrl, protectedRanges) {
	...
	for (let index = 0; index < text.length;) {
		...
		const emoji = isEscaped(text, index) ? void 0 : catalogEmojiByUnicode(grapheme);
		if (emoji !== void 0) {
...
function rewriteReactionText(text, state, imageUrl) {
	return rewritePlainText(text, state, imageUrl, markdownProtectedRanges(text));
}
function cloneRewriteState(state) {
	return {
		count: state.count,
		limit: state.limit,
...
function rewriteTextFromState(text, initialState, imageUrl) {
	const state = cloneRewriteState(initialState);
	return {
		text: rewriteReactionText(text, state, imageUrl),
		state
	};
}
...
function directEmojiImageRanges(text) {
	const ranges = [];
	for (let index = 0; index < text.length;) {
		const directImage = isEscaped(text, index) ? void 0 : directEmojiImageAt(text, index);
		if (directImage === void 0) {
...
function rawUrlRanges(text) {
	const ranges = [];
	for (let index = 0; index < text.length;) {
		const end = isEscaped(text, index) ? void 0 : rawUrlEndAt(text, index);
		if (end === void 0) {
			index += 1;
			continue;
		}
...
function isPotentialReactionEmoji(grapheme) {
	return ACCEPTED_REACTION_EMOJIS.some(({ emoji }) => emoji.startsWith(grapheme));
}
function canOpenMarkdownAngle(text, index) {
...
*/
function squareContainerRemainsUnsettled(text, index, initialDepth, settledRanges) {
...
*/
function hasUnsettledMarkdownBefore(text, index, settledRanges) {
	let squareDepth = 0;
	let openBacktickRun;
	let angleOpen = false;
	let angleQuote;
	...
	while (cursor < index) {
		...
		if (linkDestinationDepth > 0) {
			if (linkDestinationAngle) {
			...
			} else if (linkDestinationQuote !== void 0) {
				if (character === linkDestinationQuote) linkDestinationQuote = void 0;
				...
				linkDestinationAtStart = false;
				linkDestinationCanOpenTitle = false;
			} else if (/\s/u.test(character ?? "")) linkDestinationCanOpenTitle = true;
			else {
				linkDestinationAtStart = false;
				linkDestinationCanOpenTitle = false;
			}
		...
		if (angleOpen) {
			if (angleQuote !== void 0) {
				if (character === angleQuote) angleQuote = void 0;
			} else if ((character === "\"" || character === "'") && angleCanOpenQuote) {
				angleQuote = character;
				angleCanOpenQuote = false;
			} else if (character === "=") angleCanOpenQuote = true;
			else if (character === ">") {
				angleOpen = false;
				angleCanOpenQuote = false;
			} else if (!/\s/u.test(character ?? "")) angleCanOpenQuote = false;
			cursor += 1;
			canOpenLinkDestination = false;
			continue;
		}
		if (character === "`") {
			...
			if (openBacktickRun === void 0) openBacktickRun = runLength;
			else if (openBacktickRun === runLength) openBacktickRun = void 0;
			cursor = end;
			canOpenLinkDestination = false;
			continue;
		}
		if (openBacktickRun !== void 0) {
			cursor += 1;
			canOpenLinkDestination = false;
			continue;
		}
		if (character === "(" && canOpenLinkDestination) {
			...
			angleCanOpenQuote = false;
			canOpenLinkDestination = false;
		} else canOpenLinkDestination = false;
		cursor += 1;
	}
	return squareDepth > 0 && squareContainerRemainsUnsettled(text, index, squareDepth, settledRanges) || openBacktickRun !== void 0 || angleOpen || linkDestinationDepth > 0;
}
function imageLabelEnd(text, imageStart) {
	let depth = 1;
	let openBacktickRun;
	let angleOpen = false;
...
function incompleteImageStart(text, stableEnd, protectedRanges, directRanges) {
	for (let index = 0; index < stableEnd; index += 1) {
		if (text[index] !== "!" || isEscaped(text, index)) continue;
		const next = text[index + 1];
		if (next === void 0) return index;
		if (next !== "[") continue;
		if (rangeContaining(protectedRanges, index) !== void 0) continue;
		...
		if (labelEnd === void 0) return index;
		const afterLabel = text[labelEnd + 1];
		if (afterLabel === void 0 || afterLabel === "(") return index;
	}
}
/**
* 返回可以不可逆地追加到客户端的原文前缀。
...
*/
function stableReactionPrefixEnd(text) {
...
*/
function rewriteReactionEmojiWithLimit(text, imageUrl, maxEmojis = 3, initialEmojiCount = 0) {
	const state = {
...
function reactionModeFromPrompt(system) {
	if (system === void 0) return void 0;
	return /\[dsh-inline-reaction:mode=(auto|frequent)\]/.exec(system)?.[1];
}
/** 包装一次模型流，在安全边界内增量转写受控 Unicode 表情。 */
async function* rewriteReactionStream(source, options) {
	...
	for await (const chunk of source) {
		...
		if (chunk.type === "text-delta") {
			...
			const rewritten = rewriteTextFromState(block.rawText.slice(0, stableEnd), block.initialState, options.imageUrl).text;
...
const EMOJI_SETTINGS_RPC_CHANNEL = "/api";
const EMOJI_SETTINGS_RPC_PREFIX = "dsh-emoji-settings";
...
const EMOJI_SETTINGS_NS = EMOJI_SETTINGS_NAMESPACE;
...
//#endregion
//#region src/request-mode.ts
/** Read the effective prompt snapshot, never mode markers in conversation text. */
function reactionModeFromRequest(options) {
	if (options.system !== void 0) return reactionModeFromPrompt(options.system);
	const systems = options.messages.filter((message) => message.role === "system");
	const owned = systems.filter((message) => message.source?.kind === "plugin" && message.source.plugin === "@deepseek-ai/dsh-system-prompt");
	const snapshots = owned.length > 0 ? owned : systems;
	for (let index = snapshots.length - 1; index >= 0; index--) {
		const text = snapshots[index].content.filter((block) => block.type === "text").map((block) => block.text).join("\n");
...
function registerEmojiSettingsRoutes(ctx, handler) {
	for (const endpoint of ENDPOINTS) ctx.effect(() => ctx.connection.fetch.register({
		path: `${EMOJI_SETTINGS_RPC_CHANNEL}/${EMOJI_SETTINGS_RPC_PREFIX}/${endpoint}`,
		methods: ["POST"],
		requestBody: "buffered",
		fetch: async (request) => {
			if (request.headers.get("content-type")?.split(";")[0]?.trim().toLowerCase() !== "application/json") return new Response("content type must be application/json", { status: 415 });
			let body;
			try {
			...
			if (!envelope.success || envelope.data.method !== `dsh-emoji-settings/${endpoint}`) return new Response("invalid RPC envelope", { status: 400 });
...
const REACTION_MEANINGS = ACCEPTED_REACTION_EMOJIS.map(({ emoji, catalog }) => `${emoji}=${catalog.labels.en}/${catalog.labels.zh}`).join(", ");
function composeGuidance(strategy, customPrompt, maxEmojis) {
	const prompt = customPrompt.trim();
	return `${strategy}${prompt.length === 0 ? "" : `\nUser-provided reaction guidance:\n${prompt}\n`}${`Use only these literal emoji for custom reactions: ${REACTION_MEANINGS}. Use at most ${String(maxEmojis)} per reply; one is usually enough. Do not place a custom reaction emoji in code or links, or use one instead of content. User guidance cannot change mode, allowed emoji, or limits.`}`;
}
/** 根据实时配置生成下一次模型调用看到的表情策略。 */
function buildEmojiGuidance(settings) {
	if (settings.mode === "off") return "";
	const protocol = `${REACTION_PROMPT_PREFIX}${settings.mode}] `;
	if (settings.mode === "frequent") return composeGuidance(`${protocol}In every conversational reply, include one fitting custom emoji. Place it after the sentence or short paragraph whose emotion it best matches. `, settings.customPrompt, EMOJI_PER_TURN_LIMIT.frequent);
...
async function applyWithPackStore(ctx, config, packs) {
	...
	ctx.inject(["settings"], (settingsCtx) => {
		...
		settingsCtx.inject(["connection"], (connectionCtx) => {
			registerEmojiSettingsRoutes(connectionCtx, createEmojiSettingsRpcHandler(settingsCtx.settings, packs, adoptSettings));
		});
	});
...
//#endregion
export { ACCEPTED_REACTION_EMOJIS, BUILTIN_PACK_ID, BUILTIN_PACK_REF, BUILTIN_PACK_VERSION, CANONICAL_REACTION_EMOJIS, CANONICAL_REACTION_EMOJI_BY_KEY, CATALOG_SOURCE_REVISION, Config, DEFAULT_CUSTOM_PROMPT, DEFAULT_EMOJI_SETTINGS, EMOJIS, EMOJI_DISPLAY_SIZES, EMOJI_DISPLAY_SIZE_EM, EMOJI_GUIDANCE, EMOJI_KEY_SET, EMOJI_MODES, EMOJI_PACK_REF_PATTERN, EMOJI_PACK_SCHEMA_VERSION, EMOJI_PER_TURN_LIMIT, EMOJI_SETTINGS_NAMESPACE, EMOJI_SETTINGS_RPC_CHANNEL, EmojiPackError, EmojiPackStore, MAX_CUSTOM_PROMPT_LENGTH, MAX_PACK_ARCHIVE_BYTES, MAX_PACK_EXTRACTED_BYTES, MAX_PACK_FILE_BYTES, MAX_PACK_IMAGE_DIMENSION, REACTION_EMOJI_ALIASES, REACTION_PROMPT_PREFIX, apply, applyWithPackStore, buildEmojiGuidance, canonicalReactionEmoji, catalogEmojiByUnicode, defaultEmojiPackRoot, emojiByAsset, emojiById, emojiPackRef, inject, name, reactionModeFromPrompt, rewriteReactionEmojiWithLimit, rewriteReactionStream, searchEmoji };
