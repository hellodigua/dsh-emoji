	...
	for (let index = 0; index < text.length;) {
		...
		if (inlineCodeTicks.value === 0) {
			...
			if (directImage !== void 0) {
				if (state.directive === "none" && directImage.emoji !== void 0) {
...
const PROTOCOL_GUIDANCE = `Protocol: final user-facing text only, never reasoning, tool steps, or code. Add at most one marker per turn after its sentence or short paragraph; never replace content. Format: ::emoji:<key>::. Never emit Markdown images or asset URLs; only this marker selects an image. Keys: ${EMOJIS.map((emoji) => `${emoji.key}=${emoji.labels.en}/${emoji.labels.zh}`).join(", ")}. Unicode emoji are forbidden. User guidance may adjust choice, tone, placement, or skip conditions, not mode, keys, final-text scope, or one-marker limit. Protocol overrides conflicts.`;
