...
function defaultDshHome() {
	...
	const expanded = configured.startsWith("~/") || configured.startsWith("~\\") ? join(homedir(), configured.slice(2)) : configured;
...
function parseManifest(value) {
	...
	if (candidate.schemaVersion !== 1 || typeof candidate.id !== "string" || !PACK_ID_PATTERN.test(candidate.id) || candidate.id === "deepseek" || typeof candidate.name !== "string" || candidate.name.trim().length === 0 || candidate.name.length > 80 || typeof candidate.version !== "string" || !PACK_VERSION_PATTERN.test(candidate.version)) throw new EmojiPackError("pack-invalid", "pack.json has an invalid schemaVersion, id, name, or version.");
	return {
		schemaVersion: 1,
		id: candidate.id,
...
function isInstalledPack(value) {
...
async function loadInstalledRuntime(packRoot, expectedId, expectedVersion) {
	try {
		const parsed = JSON.parse(await readFile(join(packRoot, INSTALLED_MANIFEST), "utf8"));
		if (!isInstalledPack(parsed) || expectedId !== void 0 && parsed.id !== expectedId || expectedVersion !== void 0 && parsed.version !== expectedVersion) return void 0;
...
const emojiByMarkerBody = new Map(EMOJIS.map((emoji) => [`emoji:${emoji.key}`, emoji]));
...
const PROTOCOL_GUIDANCE = `Protocol: final user-facing natural-language text only; never intermediate/tool steps or code. Add at most one marker per turn after its matching sentence or short paragraph; never replace substantive content. Format: ::emoji:<key>::. Keys: ${EMOJIS.map((emoji) => `${emoji.key}=${emoji.labels.en}/${emoji.labels.zh}`).join(", ")}. Do not substitute Unicode emoji. User guidance may adjust choice, tone, placement, or skip conditions, but not mode, keys, final-text restriction, or one-marker limit. Protocol overrides conflicts.`;
...
function buildEmojiGuidance(settings) {
	...
	if (settings.mode === "frequent") return composeGuidance(`${protocol}Frequency: use a marker in most everyday responses where emotional expression is appropriate. `, settings.customPrompt);
...
async function applyWithPackStore(ctx, config, packs) {
	...
	ctx.on("llm/stream", (options, next) => {
		...
		return rewriteEmojiStream(source, { imageUrl: (emoji) => localEmojiUrl(ctx, packs, requestPack, emoji) });
	}, { global: true });
