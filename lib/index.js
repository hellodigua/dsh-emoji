...
function rewritePlainText(text, state, imageUrl, protectedRanges) {
	...
	for (let index = 0; index < text.length;) {
		...
		if (directImage !== void 0) {
			if (state.count < state.limit && directImage.emoji !== void 0) {
				state.count += 1;
...
function rewriteEmojiMarkersWithLimit(text, imageUrl, maxEmojis = 3, initialEmojiCount = 0) {
	...
	return {
		text: rewritePlainText(text, state, imageUrl, markdownProtectedRanges(text)),
...
function buildEmojiGuidance(settings) {
	...
	if (settings.mode === "frequent") return composeGuidance(`${protocol}Frequency: use markers in multiple suitable places in most everyday responses. `, settings.customPrompt, EMOJI_PER_TURN_LIMIT.frequent);
