	...
	factory: (require) => {
		...
		*/
		...
		const en = {
			...
			"mode.off.title": "Off",
			"mode.off.description": "Do not ask the AI to use custom reaction emoji or rewrite them as pack images.",
			...
			"mode.frequent.description": "Add one fitting emoji to every conversational reply and place it after the sentence or short paragraph it best matches, up to four per turn.",
		...
		const zh = {
			...
			"policy.legend": "回复策略",
			"mode.off.title": "关闭",
			"mode.off.description": "不要求 AI 使用自定义表情，也不把 Unicode 表情转写为表情包图片。",
			"mode.auto.title": "智能",
			"mode.auto.description": "仅在友好回答确实适合表达情绪时自然使用，每回合最多三张。",
			"mode.frequent.title": "高频",
			"mode.frequent.description": "在所有对话回复中加入一个合适的表情，放在最能对应当前情绪的句子或短段落后；每回合最多四张。",
			"pack.label": "表情包",
			"pack.builtin": "内置",
