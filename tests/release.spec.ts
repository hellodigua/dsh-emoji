...
function packReport(overrides: Record<string, unknown> = {}) {
  ...
  return JSON.stringify([{
    ...
    filename: 'dsh-emoji-0.2.2-beta.1.tgz',
    size: 1024,
    unpackedSize: 2048,
    files,
    ...overrides,
...
describe('release helpers', () => {
  it('要求 changelog 包含当前版本和发布日期', () => {
    expect(() => validateChangelog('## [0.2.2-beta.1] - 2026-08-15\n', '0.2.2-beta.1')).not.toThrow()
    expect(() => validateChangelog('## [0.2.1] - 2026-08-15\n', '0.2.2-beta.1')).toThrow(
      'CHANGELOG.md 缺少 0.2.2-beta.1 的日期标题',
    )
  })

  it('接受稳定版和规范的预发布 SemVer', () => {
    expect(() => validateReleaseVersion('0.2.2')).not.toThrow()
    expect(() => validateReleaseVersion('0.2.2-beta.1')).not.toThrow()
    expect(() => validateReleaseVersion('0.2.2-01')).toThrow('版本号不是可发布 SemVer')
  })
  ...
  it('校验发布包身份、运行时文件、素材数量和发布体积', () => {
    ...
    expect(validatePackReport(`build output\n${packReport()}`, 'dsh-emoji', '0.2.2-beta.1').filename).toBe('dsh-emoji-0.2.2-beta.1.tgz')
    expect(() => validatePackReport(packReport({ name: 'other' }), 'dsh-emoji', '0.2.2-beta.1')).toThrow('tarball 身份不匹配')
    expect(() => validatePackReport(packReport({ size: 7 * 1024 * 1024 }), 'dsh-emoji', '0.2.2-beta.1')).toThrow('tarball 文件名或体积异常')
  })

  it('拒绝把源码或开发素材装入 npm tarball', () => {
    const report = JSON.parse(packReport())
    report[0].files.push({ path: 'src/index.ts', size: 1, mode: 0o644 })
    expect(() => validatePackReport(JSON.stringify(report), 'dsh-emoji', '0.2.2-beta.1')).toThrow('tarball 不应包含 src/index.ts')
  })
