...
describe('Web Client inline style', () => {
  ...
  it('设置卡片提供键集合完全一致的中英文文案', () => {
    ...
    expect(emojiZh['prompt.example']).toContain('不需要时不使用')
    expect(emojiZh['mode.frequent.description']).toContain('在所有对话回复中加入一个合适的表情')
    expect(emojiEn['mode.frequent.description']).toContain('every conversational reply')
    expect(emojiZh['limit.note']).toContain('智能模式每回合最多 3 张')
    expect(emojiZh['limit.note']).toContain('多张之间必须有回复正文')
    expect(emojiEn['limit.note']).toContain('Frequent keeps up to 4')
    expect(emojiEn['limit.note']).toContain('One is usually enough')
